...
function RJMCMC_stationary_model(m_current, β_current, ω_current,
  ...
  if (m_current == 1)
    ...
    if (U <= birth_prob)
      MCMC = birth_move_stationary(m_current, β_current, ω_current, σ_current, a, b, λ, c, 
                                  ϕ_ω, ψ_ω,σ_β, ν0, γ0)
      ...
      MCMC = within_move_stationary(m_current, β_current, ω_current, σ_current, a, b, λ, c, ϕ_ω, 
                                    ψ_ω, σ_β, ν0, γ0)
      m_out = m_current
      β_out = MCMC["β"]
      ω_out = MCMC["ω"]
  ...
  elseif (m_current == n_freq_max)
    ...
    U = rand()
    if (U <= death_prob)
      MCMC = death_move_stationary(m_current, β_current, ω_current, σ_current, a, b, λ, c, ϕ_ω, ψ_ω,  σ_β, ν0, γ0)
      m_out = m_current - Int64(MCMC["accepted"])
      β_out = MCMC["β"]
      ω_out = MCMC["ω"]
      σ_out = MCMC["σ"]
    else
      MCMC = within_move_stationary(m_current, β_current, ω_current, σ_current, a, b, λ, c, ϕ_ω, ψ_ω, σ_β, ν0, γ0)
      m_out = m_current
      β_out = MCMC["β"]
      ω_out = MCMC["ω"]
      σ_out = MCMC["σ"]
    ...
    # ----- Birth
    if (U <= birth_prob)
      MCMC = birth_move_stationary(m_current, β_current, ω_current, σ_current, a, b, λ, c, ϕ_ω, ψ_ω, σ_β, ν0, γ0)
      m_out = m_current + Int64(MCMC["accepted"])
      β_out = MCMC["β"]
      ω_out = MCMC["ω"]
      σ_out = MCMC["σ"]
